...
public class ChannelController extends BaseController {
    ...
    public String view(@PathVariable Long id, ModelMap model) {
        ...
        if (view.getStatus() != Constants.STATUS_NORMAL) {

            AccountProfile profile = getProfile();
            if (profile != null)
                Assert.isTrue(profile.getId() != view.getAuthorId(), "Bài viết chưa được xuất bản");
        }

        Assert.isTrue(view.getStatus() == 0, "Bài viết không tồn tại");
