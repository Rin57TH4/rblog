...
/**
 * LightB3log utilities.
 *
 * @author <a href="http://88250.b3log.org">Liang Ding</a>
 * @version 1.9.0.6, May 9, 2019
 * @since 0.1.0
 */
public final class LightB3log {

    /**
     * Logger.
     */
    private static final Logger LOGGER = LogManager.getLogger(LightB3log.class);
     ...
     * User-Agent.
     */
    public static final String USER_AGENT_BOT = "Sym/1.0";
    ...
    static {
        try {
            ...
            if (StringUtils.isNotBlank(symPropsEnv)) {
                LOGGER.trace("Loading lightb3log.properties from env var [$SYM_PROPS=" + symPropsEnv + "]");
                resourceAsStream = new FileInputStream(symPropsEnv);
            } else {
                LOGGER.trace("Loading lightb3log.properties from classpath [/lightb3log.properties]");
                resourceAsStream = LightB3log.class.getResourceAsStream("/lightb3log.properties");
            }

            CFG.load(resourceAsStream);
        } catch (final Exception e) {
            LOGGER.log(Level.ERROR, "Loads lightb3log.properties failed, exited", e);
     ...
     */
    public static final String UPLOAD_CHANNEL = LightB3log.get("upload.channel");
     ...
     * 允许上传图片最大值，单位字节.
     */
    public static final long UPLOAD_IMG_MAX = LightB3log.getLong("upload.img.maxSize");

    /**
     * 允许上传文件（非图片）的最大值，单位字节.
     */
    public static final long UPLOAD_FILE_MAX = LightB3log.getLong("upload.file.maxSize");
    ...
    static {
        ...
        if (!LightB3log.QN_ENABLED) {
            final File file = new File(LightB3log.UPLOAD_LOCAL_DIR);
            if (!FileUtil.isExistingFolder(file)) {
                try {
                    FileUtil.mkdirs(LightB3log.UPLOAD_LOCAL_DIR);
     ...
     * 百度语音接口凭证，用于文字转语音.
     */
    public static final String BAIDU_YUYIN_API_KEY = LightB3log.get("baidu.yuyin.apiKey");

    /**
     * 百度语音接口凭证.
     */
    public static final String BAIDU_YUYIN_SECRET_KEY = LightB3log.get("baidu.yuyin.secretKey");
    ...
    /**
     * 性能监控阈值，单位毫秒，如果 {@link vn.rin} 埋点执行超过该值则会打印耗时日志.
    ...
    static {
        ...
        for (final String key : keys) {
            if (key.startsWith(prefix)) {
                final String value = LightB3log.CFG.getProperty(key);
...
//            @Override
//            public void run() {
//                final String symURL = Smart.getServePath();
...
//                        sym.put("symTitle", langPropsService.get("symphonyLabel", Smart.getLocale()));
//
