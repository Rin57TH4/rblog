                            ...
                            <li class="list-group-item " style="padding: 0 15px;">
                                ...
                                <div class="infos">
                                    ...
                                    <#--<span class="hidden-xs label label-warning">${row.channel.name}</span>-->
                                        ...
                                        <span>
                                            <#if (row.event == 1)>
                                                Thích bài viết của bạn - <a href="${base}/post/${row.postId}">${row.post.title}</a>
                                            <#elseif (row.event == 3)>
                                                Nhận xét về bài viết của bạn - <a href="${base}/post/${row.postId}">Chi tiết</a>
                                            <#elseif (row.event == 4)>
                                                Đã trả lời bình luận của bạn - <a href="${base}/post/${row.postId}">Chi tiết</a>
                                            </#if>
                                        </span>
                                    </div>
                                </div>
