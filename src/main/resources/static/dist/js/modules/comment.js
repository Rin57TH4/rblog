	...
	var Comment = {
        ...
        bindEvents : function () {
            ...
            new OwO({
                ...
                api: RIN_BLOG.BASE_PATH + '/dist/vendors/owo/OwO.json',
                position: 'down',
                width: '600px',
                maxHeight: '250px'
            });
        ...
        pageCallback: function (pn) {
        	...
        	J.getJSON(opts.load_url, {pageSize : opts.pageSize, pageNo: pn}, function (ret) {
	    		...
	    		if (ret.size < 1) {
	    			$list.append('<li><p>Chưa có đánh giá nào, hãy đến và lấy ghế sofa!</p></li>');
	    		}
	    		if (ret.totalPages > 1) {
        ...
        post: function (toId, pid, text) {
        	...
        	if (text.length == 0) {
        		layer.msg('Vui lòng nhập nội dung và gửi!', {icon: 2});
        		return false;
        	}
        	if (text.length > 255) {
        		layer.msg('Nội dung quá dài, vui lòng nhập dưới 140 ký tự', {icon: 2});
        		return false;
        	}
